...
const OVERALL_RISK_COLORS = {
  ...
  Elevated: 'text-orange-400 border-orange-500/40 bg-orange-500/10',
  High: 'text-red-400 border-red-500/40 bg-red-500/10',
};

// Renders markdown-like bold (**text**) and numbered lists
function RenderContent({ text }: { text: string }) {
  if (text.startsWith('⚠')) {
    const rest = text.slice(1).trim();
    return (
      <div className="flex items-start gap-2 mt-2 bg-amber-500/5 border border-amber-500/20 rounded-md px-3 py-2.5">
        <span className="text-amber-500 flex-shrink-0 text-base">⚠</span>
        <p className="font-body text-amber-200/70 text-xs leading-relaxed"
  ...
  if (/^\d+\./.test(text)) {
    return (
      <p className="font-body text-slate-600 text-sm leading-relaxed ml-4"
          ...
          <div className="card-base overflow-hidden">
            <div className="bg-slate-800/50 px-6 py-4 border-b border-slate-800 flex items-center justify-between">
              ...
              <button
                ...
                className="btn-ghost text-xs py-1.5 px-3 text-amber-400 hover:text-amber-300"
              >
                Load Sample Data
              </button>
            </div>
            ...
            <div className="p-6 space-y-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. Ridgeline Commons — Bldg B"
                ...
                <div>
                  ...
                  <input
                    type="text"
                    className="input-field"
                    placeholder="e.g. Commercial Mixed-Use, 5-story"
              ...
              <div>
                ...
                <input
                  type="text"
                  className="input-field"
                  placeholder="e.g. Bethlehem, PA 18015 (Northampton County)"
              ...
              <div>
                ...
                <textarea
                  className="input-field min-h-[130px] resize-y font-mono text-xs leading-relaxed"
                  placeholder="Paste boring log summaries, SPT N-values, soil classifications, and depth intervals..."
                  value={formData.boringLogs}
                  onChange={e => handleChange('boringLogs', e.target.value)}
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Observed GW depths, seasonal variability, piezometer data..."
                  value={formData.groundwater}
                  onChange={e => handleChange('groundwater', e.target.value)}
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  className="input-field min-h-[70px] resize-y"
                  placeholder="Topography, vegetation, surface drainage, adjacent structures, former use..."
                  value={formData.siteObservations}
                  onChange={e => handleChange('siteObservations', e.target.value)}
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  className="input-field min-h-[70px] resize-y"
                  placeholder="Utilities, adjacent foundations, zoning, owner requirements..."
                  value={formData.knownConstraints}
                  onChange={e => handleChange('knownConstraints', e.target.value)}
                />
              </div>

              {/* Audience */}
              <div>
                <label className="block font-body text-xs font-medium text-slate-400 mb-2 uppercase tracking-wider">
                  Report Audience
                </label>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                  {AUDIENCES.map(a => (
                    <button
                      key={a}
                      type="button"
                      onClick={() => handleChange('reportAudience', a)}
              ...
              <div className="flex items-center gap-3 pt-2">
                ...
                <button onClick={handleReset} className="btn-secondary text-xs py-3 px-4">
                  Reset
                </button>
              </div>
              ...
              {!isFormValid && (
                <p className="text-xs text-slate-600 font-body">
                  * Project Name and Boring Log data are required to generate a report.
                </p>
              )}
            </div>
          </div>
              ...
              <div className="card-base overflow-hidden">
                ...
                <div className="bg-amber-50 border-b border-amber-200 px-5 py-2.5 flex items-center gap-2">
                  ...
                  <p className="font-body text-amber-800 text-xs leading-tight">
                    <strong>Draft for engineering review only.</strong> Not a final stamped report. Requires review by a licensed Professional Engineer before use in design or construction.
                  </p>
                </div>
                ...
                <div className="bg-white p-6 max-h-[680px] overflow-y-auto">
                  {report.sections.map((section) => (
                    <div
                      key={section.id}
                      className={`mb-7 pb-7 border-b border-slate-100 last:border-0 last:mb-0 last:pb-0 ${
                        section.isDisclaimer ? 'bg-slate-50 -mx-6 px-6 py-5 rounded-b-none' : ''
                      }`}
                    >
                      {/* Section header */}
                      <div className="flex items-center gap-2 mb-3">
                        {section.isClientSummary && (
                          <span className="inline-flex px-2 py-0.5 bg-blue-100 border border-blue-200 text-blue-700 font-mono text-[9px] uppercase tracking-widest rounded">
                            Stakeholder
                          </span>
                        )}
                        {section.isDisclaimer && (
                          <span className="inline-flex px-2 py-0.5 bg-red-100 border border-red-200 text-red-700 font-mono text-[9px] uppercase tracking-widest rounded">
                            Legal Notice
                          </span>
                        )}
                        <h4 className={`font-display text-sm font-bold ${
                          section.isDisclaimer ? 'text-slate-500' : 'text-slate-900'
                        }`}>
                          {section.title}
                        </h4>
                      </div>
                      ...
                      {section.flags && section.flags.length > 0 && (
                        <div className="mt-4 space-y-2">
                          {section.flags.map((flag, i) => (
                            <div
                              key={i}
                              className={`border rounded-md px-4 py-3 ${RISK_COLORS[flag.level]}`}
                            >
                ...
                <div className="bg-slate-50 border-t border-slate-200 px-6 py-4 flex items-center justify-between gap-3">
                  <p className="font-mono text-[10px] text-slate-400 uppercase tracking-widest">
                    {report.reportId} · GeoDraft AI Draft
                  </p>
                  <div className="flex items-center gap-2">
                    <button
                      onClick={handleCopy}
